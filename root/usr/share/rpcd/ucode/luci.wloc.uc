...
function integer(value, fallback) { let number = +value; return number >= 0 ? number : fallback; }
function daemon_running() {
    ...
    } catch (e) { return system('pidof wlocd >/dev/null 2>&1') === 0; }
}
...
function status() {
    ...
    let accepted = integer(state.accepted_connections, 0), conflict = firewall.attempts > 0 && accepted === 0, reason = '';
    if (!running) {
        ...
        if (state.last_event === 'lease_failed') reason = 'Runtime rule refresh failed' + (state.last_error ? `: ${state.last_error}` : '');
        else if (state.last_event === 'cleanup_failed') reason = 'Runtime rule cleanup failed' + (state.last_error ? `: ${state.last_error}` : '');
    } else if (conflict) reason = 'transparent-proxy path conflict; redirected packets are not reaching the WLOC listener';

    let fingerprint = state.ca_fingerprint || '';
    if (!fingerprint) { let info = read_json(CAINFO) || {}; fingerprint = info.fingerprint_sha256 || ''; }
    let activity = [];
    ...
    return {
        configured, enabled, version: package_version(), running,
        rules_present: firewall.present, firewall_active: firewall.active, armed: truthy(state.armed),
        configured_aps: integer(state.configured_aps, 0), accepted_connections: accepted,
        passthrough_connections: integer(state.passthrough_connections, 0), tls_intercepted: integer(state.tls_intercepted, 0),
        wloc_requests: integer(state.wloc_requests, 0), patched_responses: integer(state.patched_responses, 0),
        delivered_responses: integer(state.delivered_responses, 0), patch_failures: integer(state.patch_failures, 0),
        last_event: `${state.last_event || ''}`, last_error: `${state.last_error || ''}`, service_reason: reason,
        session_started_at: integer(state.session_started_at, 0), updated_at: integer(state.updated_at, 0),
        interception_attempts: firewall.attempts, intercepted_packets: firewall.intercepted, path_conflict: conflict,
        fingerprint, profile_url: '/wloc-ca.mobileconfig', ap_activity: activity
    };
}
function configured_access_points() {
    let ctx = cursor(), hostapd = run_command("ubus -S list 'hostapd.*' 2>/dev/null"), active = {}, rows = [];
    ...
    try {
        ctx.foreach('wireless', 'wifi-iface', function(section) {
            let ifname = `${section.ifname || ''}`, mode = `${section.mode || 'ap'}`;
            if (!match(ifname, /^[A-Za-z0-9_.-]{1,15}$/) || (mode !== 'ap' && mode !== 'ap-wds')) return;
            push(rows, {
                section: `${section['.name'] || ''}`, iface: ifname, ssid: `${section.ssid || ''}`,
                disabled: truthy(section.disabled), active: active[ifname] === true
            });
        });
    } catch (e) {}
...
function regenerate_ca() {
    ...
    try { enabled = truthy(ctx.get('wloc', 'main', 'enabled')); } catch (e) {}
    if (!enabled) return { ok: false, error: 'Unable to regenerate CA while WLOC is disabled.', error_code: 'service_disabled', detail: 'Enable WLOC before regenerating the CA.' };

    let cleanup = run_ucode(RULES, [ 'cleanup' ]);
    if (!cleanup.ok) return { ok: false, error: 'Unable to clean up WLOC firewall rules.', error_code: 'cleanup_failed', detail: cleanup.error || 'The WLOC firewall cleanup command failed.' };

    let old_info = read_json(CAINFO) || {}, old_fingerprint = old_info.fingerprint_sha256 || '';
    if (system(`${INIT} stop >/dev/null 2>&1`) !== 0) return { ok: false, error: 'Unable to regenerate the WLOC Root CA.', error_code: 'ca_regeneration_failed', detail: 'Unable to stop the WLOC service.' };
    for (let path in [ CA_KEY, CA_DER, CA_PEM, CAINFO, CA_PROFILE ]) unlink(path);
    if (system(`${INIT} start >/dev/null 2>&1`) !== 0) return { ok: false, error: 'Unable to regenerate the WLOC Root CA.', error_code: 'ca_regeneration_failed', detail: 'WLOC did not restart after CA removal.' };
    ...
    for (let attempt = 0; attempt < 15; attempt++) {
        if (daemon_running()) {
            ...
            if (fingerprint && (!old_fingerprint || fingerprint !== old_fingerprint) && read_file(CA_KEY) != null && read_file(CA_DER) != null && read_file(CA_PEM) != null && read_file(CA_PROFILE) != null)
                return { ok: true, status: 'ready', fingerprint, profile_url: '/wloc-ca.mobileconfig' };
        }
        system('sleep 1');
    }
    return { ok: false, error: 'Unable to regenerate the WLOC Root CA.', error_code: 'ca_regeneration_failed', detail: fingerprint === old_fingerprint && old_fingerprint ? 'The replacement CA fingerprint did not change.' : 'The replacement CA was not generated before the timeout.' };
}

const methods = {
    status: { args: {}, call: function() { return status(); } },
    configured_access_points: { args: {}, call: function() { return configured_access_points(); } },
    regenerate_ca: { args: {}, call: function() { return regenerate_ca(); } }
};
