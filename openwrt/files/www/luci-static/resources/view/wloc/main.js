...
return view.extend({
	...
	render: function(data) {
		...
		clients.sortable = true;
		clients.nodescriptions = true;
		clients.addbtntitle = _('Add device rule');

		option = clients.option(form.Flag, 'enabled', _('Enabled'));
		option.default = '1';
		option.rmempty = false;

		option = clients.option(form.Value, 'name', _('Name'));
		option.placeholder = _('My iPhone');
		option.rmempty = false;

		var macOption = clients.option(form.Value, 'mac', _('Source MAC'));
		...
		macOption.validate = function(sectionId, value) {
			...
			var duplicate = uci.sections('wloc', 'client').some(function(rule) {
				return rule['.name'] !== sectionId && String(rule.mac || '').toLowerCase() === value;
			});
			return duplicate ? _('This MAC address is already used by another rule.') : true;
		};
		...
		var proxyHostOption = clients.option(form.Value, 'proxy_host', _('Proxy host'));
		...
		var proxyPortOption = clients.option(form.Value, 'proxy_port', _('Proxy port'));
		...
		var lastUpdatedOption = clients.option(form.DummyValue, '_last_updated', _('Last updated'));
		...
		var lookupResultOption = clients.option(form.DummyValue, '_lookup_result', _('Lookup result'));
